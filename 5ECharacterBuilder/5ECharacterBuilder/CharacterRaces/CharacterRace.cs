...
{
    internal class CharacterRace : ICharacter
    {
        ...
        public int SkillBonus(Skill skill, int? profBonus) { return _character.SkillBonus(skill, profBonus ?? ProficiencyBonus); }
        //public void LevelUp(Class cclass) { _character.LevelUp(cclass); }
        public void LevelDown() { _character.LevelDown(); }
        public virtual int CalculateMaxHp(HitDice hitDice) => _character.CalculateMaxHp(hitDice);

        internal void AddRaceFeature(string feature) { RaceFeatures.Add( CharacterData.RaceFeatures.First(rf => rf.Name == feature)); }
    }
}